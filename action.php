...
if (isset($_POST["getProduct"])) {
	...
	$product_query = "SELECT * FROM products,category WHERE product_cat=cat_id LIMIT $start,$limit";
		...
		echo "
                        ...
                        <div class='col-md-4 col-xs-6'>
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='add-to-cart'>
										<button pid='$pro_id' id='product' href='#' tabindex='0' class='add-to-cart-btn'><i class='fa fa-shopping-cart'>
...
if (isset($_POST["addToCart"])) {
	...
	if (isset($_SESSION["uid"])) {
		...
		if ($count > 0) {
			...
			VALUES ('$p_id','$ip_add','$user_id','1','completed')";
...
//Cart Item From Database to Dropdown menu
if (isset($_POST["Common"])) {

	if (isset($_SESSION["uid"])) {
		//When user is logged in this query will execute
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a
	,cart b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
	} else {
		//When user is not logged in this query will execute
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,
        cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
	}
	$query = mysqli_query($con, $sql);
			...
			while ($row = mysqli_fetch_array($query)) {
				...
				echo '
                    ...
                    <div class="product-widget">
												<div class="product-img">
													<img src="book_images/' . $product_image . '" alt="">
												</div>
												
												<div class="product-body">
													<h3 class="product-name">
													<a href="#">' . $product_title . '</a></h3>
...
?>
	...
	if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			// "Ready to checkout" button if user is not login
			echo '<div style="width:100%" class="container main main-raised ">
						...
						<tr>
							<td data-th="Product">
								<div class="row">
									...
									<iframe src="book_images/' . $product_image . '" style="height: 70px;width:75px;"/> </iframe></div>
									<div class="col-sm-10">
										<h4 class="nomargin product-name header-cart-item-name">
										<a href="#">' . $product_title . '</a></h4>
										<p><i>That price is only for special customers</i></p>
									</div>
								</div>
							</td>
			...
			if (!isset($_SESSION["uid"])) {
				...
				echo '
						...
						<form action="checkout.php" method="post">
							...
							<input type="hidden" name="custom" value="admin@gmail.com">
				...
				echo
				'<input type="hidden" name="return" value="http://localhost/class1/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/class1/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/class1/cancel.php"/>
